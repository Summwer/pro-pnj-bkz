...
#     if i < aux.insert_left_bound or nlen >= olen or i > aux.kappa+aux.down_stop:
#         return False
#     return log(olen / nlen) - i * log(aux.prefer_left_insert)


def scoring_down(i, nlen, olen, aux):
    if(i >= aux.insert_left_bound):
        return log( olen / nlen ) - i * log(aux.prefer_left_insert)
    else:
        return False
...
def pump(g6k, tracer, kappa, blocksize, dim4free, down_sieve=False, down_sat=None, # Main parameters
         ...
         increasing_insert_index=True, prefer_left_insert=2, 
         #prefer_left_insert=1.04,                                     # Insertion policy
         verbose=False,                                                                             # Misc
            ...
            while (g6k.n > 5): #and (pump.insert_left_bound <= kappa+down_stop):
                with tracer.context(("pump-step-down", "l:%d r:%d n:%d" % (g6k.l, g6k.r, g6k.n))):
                    with g6k.temp_params(saturation_ratio=pump.saturation_ratio_down):
                        ...
                        if(ii is None or ii > kappa+down_stop):
                            break  
