...
def lwe_kernel(arg0, params=None, seed=None):
    ...
    # log2_rr = [(log2(g6k.M.get_r(i,i))/2.)-log2(sigma) for i in range(d)]
    # print(log2_rr)
    ...
    if(strategy_method == "bssa"):
        bssa = BSSA(d)
        T0_bssa = time.time()
        bssa(log2_rr)
        print("Cost for generate strategy through BSSA: %.2f sec" %(time.time()-T0_bssa))
