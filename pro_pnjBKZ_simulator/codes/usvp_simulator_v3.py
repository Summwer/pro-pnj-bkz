...
from .pump_estimation import pump_estimation2 as pump_estimation

"""v3: generate optimal strategy from pro-bkz based on an enumeration algorithm
Use pump cost as basis quality"""
...
def max_tour_for_pnjbkz_beta(BS,sslope,beta,jump,MAX_TIME,q,alpha,succ_prob):
    log_rr0 = BS[sslope][0]
    d = len(log_rr0)
    loop = 0
    local_time = 0
    ...
    pump_time,_,_,_= pump_estimation(sim_log_rr1,q,alpha,succ_prob)
    sim_slope1 = - 2**pump_time
...
def gen_bkzstrategy_v3(file,BS,sslope, d , jump, MAX_TIME,q,alpha,succ_prob):
    ...
    #Initialize BS
    for beta in range(max(50,sbeta+1),d):
        BS= max_tour_for_pnjbkz_beta(BS,sslope,beta,jump,MAX_TIME,q,alpha,succ_prob)
    ...
    #     sslope = bs[0]
    #     BS = gen_bkzstrategy_v3(file,BS,sslope,target_norm, d , jump,MAX_TIME)
    
    return BS

#A strategy for all of the situations about diffection redcution.
def gen_bkzstrategy_min_cost_v3(file_path,n,alpha,log_rr0, d, jump, goal_margin,MAX_TIME,q,succ_prob):
    ...
    file.write("New Strategy:\nn = %d, alpha = %s, jump = %d, goal_margin = %.2f, MAX_TIME = %.2f h" %(n,alpha,jump,goal_margin,MAX_TIME/3600.))
    file.write("\n")
    ...
    pump_time,_,_,_= pump_estimation(log_rr0,q,alpha,succ_prob)
    sslope = - 2**pump_time
    BS = {sslope: (log_rr0,0,[],[],1)}#store the intial slope (slope) = (sim_logrr0,T,blocksize_strategy)  
    
    BS = gen_bkzstrategy_v3(file,BS,sslope,d, jump,MAX_TIME,q,alpha,succ_prob)
    ...
    for bs in BS:
        log_rr = BS[bs][0]
        local_cost = BS[bs][1]
        pump_time,llb,n_max,f = pump_estimation(log_rr,q,alpha,succ_prob)
        pump_cost = 2**pump_time
        total_cost = local_cost + pump_cost
        slope = bs
        #file.write("Timepump(%d,%d,%d) = %f s" %(llb,n_max,f,pump_cost))
        #file.write(BS[bs][2],total_cost)
        if min_cost > total_cost and n_max-f < 300: #and  slope >= target_slope + (jump-1)*0.0003:
            llb_min = llb
            n_max_min = n_max
            f_min = f
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path):
        ...
        os.mkdir(file_path)
    blocksize_strategy, bkz_cost, pump_cost, total_cost,t_gen = gen_bkzstrategy_min_cost_v3(file_path+"/new_strategy",n,alpha,log_rr0, d, jump, goal_margin,ebeta,MAX_TIME)
