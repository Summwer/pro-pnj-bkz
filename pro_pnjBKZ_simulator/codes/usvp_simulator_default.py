...
import time
import os
from .util import predict_slope_with_beta, get_current_slope,dim4free_wrapper,default_dim4free_fun
from .pnjbkz_simulator import simulatepnjbkzloop
from .pump_estimation import pump_estimation1,pump_estimation2
from .pump_simulator import calculate_sim_log_gs_lengths_for_pump
from .util import gaussian_heuristic,get_current_slope
from scipy.special import chdtr

'''Strategy in Default G6K'''


#compute sieve dim  where actual sieve dim = sieve_dim - f
def compute_blocksize(d,beta,f):
    for blocksize in range(d):
        dim4f = dim4free_wrapper(default_dim4free_fun, blocksize)
        if blocksize - dim4f >= beta -f:
            return blocksize,dim4f



def usvp_simulator_default(file_path,n,alpha,log_rr0,target_norm,b,goal_margin,q,succ_prob):
    ...
    # blocksizes = list(range(10, 50)) + list(reversed(range(b-14, 60, -10))) + list(range(b - 12, b + 25, 2)) #g6k-gpu
    blocksizes = list(range(10, 50)) + [b-20, b-17] + list(range(b - 14, b + 100, 2)) #g6k-cpu
    # print(blocksizes)
    local_cost = 0
    pump_cost = 0
    total_cost = 0
    ...
    for beta in blocksizes:
        ...
        log_rr, pre_pnj_time = simulatepnjbkzloop(log_rr, beta, 1, d,1)
        slope = get_current_slope(log_rr,0,d)
        blocksize_strategy +=" %d (%.3f s, %.5f) -> " %(beta, pre_pnj_time,slope)
        local_cost += pre_pnj_time
        total_cost = local_cost
        pump_cost1,llb1,blocksize1,f1 = pump_estimation1(log_rr,q,alpha)
        

        _,llb2,blocksize2,f2 = pump_estimation2(log_rr,q,alpha,succ_prob=succ_prob)
        ...
        if pump_cost1 <= total_cost:
            beta1,f = compute_blocksize(d,blocksize1,f1)
            log_rr = calculate_sim_log_gs_lengths_for_pump(log_rr,d,d-beta1,f) #log_rr after pump_simulator
            GH = gaussian_heuristic(log_rr[d-beta1:])
            length=(GH/(sigma**2))
            p = chdtr(beta1, length)   
            # print(blocksize1,f1,blsocksize2,f2,blocksize1-f1,blocksize2-f2)
            if p >= succ_prob:
                pump_cost += pump_cost1
                blocksize_strategy += " pump(%d,%d,%d) (%.3f s) ]" %(llb1,d-llb1,f1,pump_cost1)
                flag = True
                break
            else:
                local_cost = local_cost 
                pump_cost += pump_cost1
                if beta1 == blocksizes[-1]:
                    slope = get_current_slope(log_rr,0,d)
                    blocksize_strategy += " pump(%d,%d,%d) (%.3f s, %.5f) " %(llb1,d-llb1,f1,pump_cost1,slope)
                else:
                    slope = get_current_slope(log_rr,0,d)
                    blocksize_strategy += " pump(%d,%d,%d) (%.3f s, %.5f) ->" %(llb1,d-llb1,f1,pump_cost1,slope)
    ...
    file.write("\n")
    file.write("Time cost for pump = %.2f h," %((total_cost - local_cost)/3600.))
    file.write("\n")
